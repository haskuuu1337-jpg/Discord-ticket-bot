...
let config = loadJson(CONFIG_PATH, {
  ...
  fakeInviteHours: 24,
  defaultBannerUrl: "",
  welcomeMessageTemplate: "{member} został zaproszony przez {inviter}, który posiada teraz **{netInvites}** zaproszenie(a)! 🎉",
  currency: {
    buyRateKPer1zl: 7,
    sellRateZlPer100k: 7,
    paymentMethods: {
...
let panelsDb = loadJson(PANELS_DB_PATH, { statsPanelMessageId: "", calculatorPanelMessageId: "" });
  ...
  return [
    new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
        ...
        .addOptions(
          new StringSelectMenuOptionBuilder().setLabel("Kurs kupna").setDescription("1 zł = X k").setValue("buyRate"),
          new StringSelectMenuOptionBuilder().setLabel("Kurs sprzedaży").setDescription("100k = X zł").setValue("sellRate"),
          new StringSelectMenuOptionBuilder().setLabel("Prowizje metod").setDescription("Zmień prowizję dla metody").setValue("commissions"),
          new StringSelectMenuOptionBuilder().setLabel("Banner powitania").setDescription("Zmień URL banneru").setValue("banner")
        )
    )
  ];
}
...
async function syncGuildInvites(guild) {
  try {
    ...
    inviteCache.set(guild.id, new Collection(fetched.map(invite => [invite.code, invite.uses])));
    return fetched;
  } catch (error) {
...
function calculateSell(kAmount, paymentMethod) {
  const commission = config.currency.paymentMethods[paymentMethod] ?? 0;
  const grossZl = (kAmount / 100) * config.currency.sellRateZlPer100k;
  const netZl = grossZl * (1 - commission / 100);
  ...
  try {
    if (interaction.isChatInputCommand()) {
      if (interaction.commandName === "invites") {
        ...
        return await interaction.reply({ embeds: [formatInviteStats(user.tag, stats)], ephemeral: true });
      }
      ...
      if (interaction.commandName === "invitetop") {
        ...
        const lines = await Promise.all(ranking.map(async (entry, index) => {
          try {
      ...
      if (interaction.commandName === "inviteadd") {
        ...
        return await interaction.reply({ content: `✅ Dodano **${amount}** bonusowych zaproszeń dla ${user}.`, ephemeral: true });
      }
      ...
      if (interaction.commandName === "inviteremove") {
        ...
        return await interaction.reply({ content: `✅ Odjęto **${amount}** bonusowych zaproszeń dla ${user}.`, ephemeral: true });
      }
      ...
      if (interaction.commandName === "invitereset") {
        ...
        return await interaction.reply({ content: `✅ Zresetowano statystyki ${user}.`, ephemeral: true });
      }
    ...
    if (interaction.isButton()) {
      if (interaction.customId === "stats:self") {
        ...
        return await interaction.reply({ embeds: [formatInviteStats(interaction.user.tag, stats)], ephemeral: true });
      }
      ...
      if (interaction.customId === "calc:admin") {
        if (!interaction.memberPermissions.has(PermissionsBitField.Flags.Administrator)) {
          return await interaction.reply({ content: "❌ Tylko administrator może otworzyć ten panel.", ephemeral: true });
        }
    ...
    if (interaction.isModalSubmit()) {
      if (interaction.customId === "modal:buy" || interaction.customId === "modal:sell") {
        const mode = interaction.customId.split(":")[1];
        const amountRaw = interaction.fields.getTextInputValue("amount");
        tempCalcSessions.set(interaction.user.id, { mode, amountRaw });
      ...
      if (interaction.customId === "admin:set_buyRate") {
        ...
        persistAll();
        return await interaction.reply({ content: `✅ Ustawiono kurs kupna: 1 zł = ${value}k`, ephemeral: true });
      }
      ...
      if (interaction.customId === "admin:set_sellRate") {
        ...
        persistAll();
        return await interaction.reply({ content: `✅ Ustawiono kurs sprzedaży: 100k = ${value} zł`, ephemeral: true });
      }
      ...
      if (interaction.customId === "admin:set_banner") {
        ...
        persistAll();
        return await interaction.reply({ content: "✅ Zmieniono banner powitania.", ephemeral: true });
      }
      ...
      if (interaction.customId.startsWith("admin:set_commission:")) {
        ...
        const value = Number(interaction.fields.getTextInputValue("value").replace(",", "."));
        if (!Number.isFinite(value) || value < 0) {
          return await interaction.reply({ content: "❌ Podaj poprawny procent prowizji.", ephemeral: true });
        }
        config.currency.paymentMethods[paymentMethod] = value;
        persistAll();
        return await interaction.reply({ content: `✅ Ustawiono prowizję dla **${paymentMethod}** na **${value}%**.`, ephemeral: true });
      }
    }
    ...
    if (interaction.isStringSelectMenu()) {
      if (interaction.customId === "calc:payment_select") {
        ...
        if (!session) {
          return await interaction.update({ content: "❌ Sesja wygasła. Kliknij przycisk jeszcze raz.", components: [] });
        }
        ...
        if (session.mode === "buy") {
          ...
          if (!Number.isFinite(zl) || zl <= 0) {
            ...
            return await interaction.update({ content: "❌ Podano niepoprawną kwotę w zł.", components: [] });
          }
        ...
        if (session.mode === "sell") {
          const kAmount = parseFlexibleAmountToK(amountRaw);
          if (!Number.isFinite(kAmount) || kAmount <= 0) {
            tempCalcSessions.delete(interaction.user.id);
            return await interaction.update({ content: "❌ Podano niepoprawną ilość waluty.", components: [] });
          }

          const result = calculateSell(kAmount, method);
          tempCalcSessions.delete(interaction.user.id);
          ...
          return await interaction.update({
            content:
              ...
              `• Metoda płatności: **${method}**\n` +
              `• Prowizja: **${result.commission}%**\n` +
              `• Kwota przed prowizją: **${result.grossZl.toFixed(2)} zł**\n` +
              `• Dostaniesz: **${result.netZl.toFixed(2)} zł**`,
            components: []
          });
        }
      }
      ...
      if (interaction.customId === "admin:select_setting") {
        if (!interaction.memberPermissions.has(PermissionsBitField.Flags.Administrator)) {
          return await interaction.update({ content: "❌ Tylko administrator może używać tego panelu.", components: [] });
        }
    ...
    if (interaction.deferred || interaction.replied) {
      await interaction.followUp({ content: "❌ Wystąpił błąd podczas obsługi akcji.", ephemeral: true }).catch(() => {});
    } else {
      await interaction.reply({ content: "❌ Wystąpił błąd podczas obsługi akcji.", ephemeral: true }).catch(() => {});
    }
  }
});
